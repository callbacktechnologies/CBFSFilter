...
class RegMon:
    ...
    def initialize(self):
        ...
        reg_filter = self.create_filter()
        self.check_driver(reg_filter)
        self.dispose_filter(reg_filter)
    ...
    def add_event_listeners(self, reg_filter):
        reg_filter.on_after_close_key = self.after_close_key
        reg_filter.on_after_create_key = self.after_create_key
        reg_filter.on_after_delete_key = self.after_delete_key
        reg_filter.on_after_delete_value = self.after_delete_value
        reg_filter.on_after_enumerate_key = self.after_enumerate_key
        reg_filter.on_after_enumerate_value = self.after_enumerate_value
        reg_filter.on_after_get_key_security = self.after_get_key_security
        reg_filter.on_after_open_key = self.after_open_key
        reg_filter.on_after_query_key = self.after_query_key
        reg_filter.on_after_query_value = self.after_query_value
        reg_filter.on_after_rename_key = self.after_rename_key
        reg_filter.on_after_set_key = self.after_set_key
        reg_filter.on_after_set_value = self.after_set_value
        reg_filter.on_before_create_key = self.before_create_key
        reg_filter.on_before_delete_value = self.before_delete_value
        reg_filter.on_before_open_key = self.before_open_key
        reg_filter.on_before_rename_key = self.before_rename_key
        reg_filter.on_before_set_key = self.before_set_key
        reg_filter.on_before_set_key_security = self.before_set_key_security
        reg_filter.on_before_set_value = self.before_set_value
        reg_filter.on_cleanup_key_context = self.cleanup_key_context
        reg_filter.on_close_key_handle = self.close_key_handle
        reg_filter.on_error = self.error
        reg_filter.on_worker_thread_creation = self.worker_thread_creation
        reg_filter.on_worker_thread_termination = self.worker_thread_termination

    @staticmethod
    def dispose_filter(reg_filter):
    ...
    def check_driver(self, reg_filter):
        try:
            status = reg_filter.get_driver_status(self.PRODUCT_GUID)
            ...
            if status is None:
                ...
                version = reg_filter.get_driver_version(self.PRODUCT_GUID)
    ...
    def install(self, file_name):
        ...
        try:
            print("Installing the driver from '{}'".format(file_name))
            rebootNeeded = filter.install(file_name, self.PRODUCT_GUID, None, cbfsfilter.INSTALL_REMOVE_OLD_VERSIONS, None)
    ...
    def nt_keyname_to_winapi_keyname(self, name):
        if name is None or len(name) == 0:
            return ""

        lowered_name = name.lower()

        if RegMon.KEY_HKLM.startswith(lowered_name):
            return "HKLM"

        if RegMon.KEY_HKCU.startswith(lowered_name):
            return "HKCU"

        if RegMon.KEY_HKCR.startswith(lowered_name):
            return "HKCR"

        if RegMon.KEY_HKU.startswith(lowered_name):
            return "HKU"

        if lowered_name.startswith(RegMon.KEY_HKLM):
            return "HKLM\\" + name[len(RegMon.KEY_HKLM):]

        if lowered_name.startswith(RegMon.KEY_HKCU):
            return "HKCU\\" + name[len(RegMon.KEY_HKCU):]

        if lowered_name.startswith(RegMon.KEY_HKCR):
            return "HKCR\\" + name[len(RegMon.KEY_HKCR):]

        if lowered_name.startswith(RegMon.KEY_HKU):
    ...
    def log(self, operation, name, result, details):
        ...
        print("<LogInfo> Number: {}, Time Elapsed: {}, Operation: {}, Name: {}, Result: {}, Details: {}".format(
            number, elapsed, operation, self.nt_keyname_to_winapi_keyname(name), self.result_to_str(result), details))

    def is_bit_set(self, value, bit):
        return (value & bit) != 0

    def value_type_to_str(self, value_type):
        if value_type == cbfsfilter.REG_VALUETYPE_SZ:
            return "SZ"

        if value_type == cbfsfilter.REG_VALUETYPE_EXPAND_SZ:
            return "EXPAND_SZ"

        if value_type == cbfsfilter.REG_VALUETYPE_BINARY:
            return "BINARY"

        if value_type == cbfsfilter.REG_VALUETYPE_DWORD:
            return "DWORD"

        if value_type == cbfsfilter.REG_VALUETYPE_MULTI_SZ:
            return "MULTI_SZ"

        if value_type == cbfsfilter.REG_VALUETYPE_QWORD:
            return "QWORD"

        return f"unknown ({value_type})"

    def replace_char(self, s, original, replace_with):

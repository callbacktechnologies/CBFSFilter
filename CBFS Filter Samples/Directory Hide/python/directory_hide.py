...
FAKE_ALTITUDE_FOR_DEBUG = "360000"
...
def banner():
    ...
    print("Application shows how to hide a directory from access by other programs.")
...
def check_driver():
    filter = CBFilter()
    state = filter.get_driver_status(PROGRAM_NAME)
    if state == MODULE_STATUS_NOT_PRESENT:
        print("CBFilter driver not installed")
    else:
        version = filter.get_driver_version(PROGRAM_NAME)
...
def install():
    ...
    filter = CBFilter()
    try:
        reboot_needed = filter.install(cab_file, PROGRAM_NAME, None, FAKE_ALTITUDE_FOR_DEBUG,
    ...
    except CBFSFilterCbfilterError as err:
...
def uninstall():
    ...
    try:
        reboot_needed = filter.uninstall(cab_file, PROGRAM_NAME, None, UNINSTALL_VERSION_ALL)
...
def hide():
    global folder_path_to_hide, folder_name_to_hide
